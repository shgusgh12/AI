...
x_train=x_train[np.isin(y_train,[9])] # 5번 부류는 ankle boot
...
for i in range(1, epochs+1): # 학습을 수행
    ...
    if i == epochs:
      ...
      fake_accuracy = discriminator.evaluate(generator.predict(np.random.normal(0, 1, (len(x_train), zdim))), np.zeros((len(x_train), 1)), verbose=0)[1]
      print("실제 데이터 분별 정확도:", real_accuracy)
      print("가짜 데이터 분별 정확도:", fake_accuracy)
