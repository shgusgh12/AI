...
history = model.fit(x_train, x_train, epochs=5, batch_size=128, shuffle=True, validation_data=(x_test, x_test))

# 마지막 epoch에서의 validation 데이터셋의 mse loss 값
last_loss = history.history['val_loss'][-1]
print("마지막 epoch에서의 validation 데이터셋의 MSE loss 값:", last_loss)

# x_test 원본 이미지와 생성된 이미지 10개씩 출력
n = 10
...
for i in range(n):
    ...
    decoded_img = model.predict(np.expand_dims(x_test[i], axis=0))
